...
- 直接调用深层模块
- 侧信道攻击（时序、功耗等）
- **重放 credential（replay 攻击）**：Phase 1-2 使用静态 credential，不维护状态，不检测 replay。Replay 防御留到 Phase 3-4（生产部署阶段）实现。
...
- **Replay 攻击防御**：Phase 1-2 使用静态 credential，不维护状态，不检测重放。研究重点是验证"LWE 验证可以编译为神经网络"，replay 防御的复杂性（状态管理、持久化、并发控制、训练/推理分离）超出当前核心目标。Replay 防御将在 Phase 3-4（生产部署阶段）使用 Challenge-Response 或 Time-based Nonce 实现；
