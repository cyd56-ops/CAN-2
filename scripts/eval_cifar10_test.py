...
def _verify_integrity(args: argparse.Namespace, checkpoint: Path) -> tuple[str, str, dict]:
...
def _aggregate(paths: list[str], output: Path, force: bool) -> None:
    ...
    metrics = {}
    for section in ("authorized", "unauthorized"):
        for key, value in records[0].get(section, {}).items():
            if isinstance(value, (int, float)):
                values = [r[section][key] for r in records]
                metrics[f"{section}.{key}"] = {"values": values, "mean": float(np.mean(values)), "std": float(np.std(values))}
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps({"schema_version": 1, "seeds": [r["seed"] for r in records], "metrics": metrics}, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    if manifest_entry:
        ...
        if manifest_entry.get("seed") != config_seed or manifest_entry.get("stage", "").upper() != stage or manifest_entry.get("epoch") != epoch:
            raise ValueError("Manifest 的 seed/stage/epoch 与 checkpoint metadata 不一致")
    provenance = "partial"
    summary = None
    if args.summary:
        summary = json.loads(Path(args.summary).read_text(encoding="utf-8"))
        required = (summary.get("split_hash"), summary.get("keypair", {}).get("A_sha256"), summary.get("keypair", {}).get("b_sha256"))
    ...
    dataset = CIFAR10WithCoarse(args.data_root, train=False, transform=get_cifar_transforms(False), download=False)
    ...
    if not 0.0 < args.mixed_ratio < 1.0 or tail_valid < 2 or tail - tail_valid < 1:
        raise ValueError("mixed-ratio 在尾批下必须保证至少两个 valid 和一个 invalid 样本")
    ...
    result = TestSplitEvaluator(model, generator, params, device, args.mixed_ratio).evaluate(loader)
    if args.measure_latency:
        ...
        result["latency"] = {"measured": True, "scope": "forward_and_routing", "batch_size": 256, **TestSplitEvaluator(model, generator, params, device, args.mixed_ratio).measure_latency(latency_images)}
    else:
    ...
    result.update({
        ...
        "device": str(device),
        "checkpoint": {"path": str(checkpoint), "sha256": actual_hash, "integrity_check": integrity, "stage": str(payload.get("stage", "")).upper(), "epoch": int(payload.get("epoch", -1))},
        "keypair": {"A_sha256": hashlib.sha256(A.tobytes()).hexdigest(), "b_sha256": hashlib.sha256(b.tobytes()).hexdigest(), "rng_scheme": "numpy.default_rng(seed + 100)", "rng_version": 1, "rng_scheme_source": rng_source},
        "credential_rng_seed": seed + 500,
        "credential_rng_note": "评估使用显式派生 seed + 500；不构成统计或密码学独立性证明",
        "training_credential_rng_seed": metadata.get("training_credential_rng_seed"),
        "lwe": metadata["lwe"],
        "lwe_provenance": "complete" if {"q", "secret_bound"}.issubset(metadata["lwe"]) else "partial_legacy_defaults",
        "dataset": {"dataset_name": "CIFAR10", "split": "test", "train_flag": False, "size": len(dataset)},
        "leakage_check": {"level": "split_identity_and_metadata_only", "split_identity_verified": True, "train_size": len(split.get("train_indices", [])) or split.get("train_size"), "val_size": len(split.get("validation_indices", [])) or split.get("val_size"), "split_hash": split.get("split_hash") if summary is not None else None, "split_hash_matches_summary": True if summary is not None else None},
